export function MicrosoftLogo({ className = "h-11 w-auto" }: { className?: string }) {
  return (
    <svg viewBox="0 0 216 44" className={className}>
      <g transform="translate(0,4)">
        <rect x="0" y="0" width="16" height="16" fill="#F25022" />
        <rect x="18" y="0" width="16" height="16" fill="#7FBA00" />
        <rect x="0" y="18" width="16" height="16" fill="#00A4EF" />
        <rect x="18" y="18" width="16" height="16" fill="#FFB900" />
      </g>
      <text x="46" y="30" fontSize="26" fontWeight="600" fill="white" fontFamily="'Geist Sans', system-ui, sans-serif">
        Microsoft
      </text>
    </svg>
  );
}

export function AWSLogo({ className = "h-11 w-auto" }: { className?: string }) {
  return (
    <svg viewBox="0 0 100 48" className={className}>
      <text x="8" y="30" fontSize="32" fontWeight="900" fill="white" fontFamily="'Geist Sans', system-ui, sans-serif" letterSpacing="2">
        AWS
      </text>
      <path d="M10 40 Q50 48 90 40" fill="none" stroke="#FF9900" strokeWidth="3" strokeLinecap="round" />
    </svg>
  );
}

export function GoogleCloudLogo({ className = "h-11 w-auto" }: { className?: string }) {
  return (
    <svg viewBox="0 0 280 48" className={className}>
      {/* Official Google Cloud mark */}
      <g transform="translate(2,4) scale(0.625)">
        <path d="M40.728 20.488l2.05.035 5.57-5.57.27-2.36C44.2 8.657 38.367 6.26 31.993 6.26c-11.54 0-21.28 7.852-24.163 18.488.608-.424 1.908-.106 1.908-.106l11.13-1.83s.572-.947.862-.9A13.88 13.88 0 0132 17.375c3.3.007 6.34 1.173 8.728 3.102z" fill="#EA4335" />
        <path d="M56.17 24.77c-1.293-4.77-3.958-8.982-7.555-12.177l-7.887 7.887c3.16 2.55 5.187 6.452 5.187 10.82v1.392c3.837 0 6.954 3.124 6.954 6.954 0 3.837-3.124 6.954-6.954 6.954H32.007L30.615 48v8.346l1.392 1.385h13.908A18.11 18.11 0 0064 39.647c-.007-6.155-3.1-11.6-7.83-14.876z" fill="#4285F4" />
        <path d="M18.085 57.74h13.9V46.6h-13.9a6.89 6.89 0 01-2.862-.622l-2.007.615-5.57 5.57-.488 1.88a18 18 0 0010.926 3.689z" fill="#34A853" />
        <path d="M18.085 21.57A18.11 18.11 0 000 39.654c0 5.873 2.813 11.095 7.166 14.403l8.064-8.064a6.96 6.96 0 01-4.099-6.339c0-3.837 3.124-6.954 6.954-6.954 2.82 0 5.244 1.7 6.34 4.1l8.064-8.064c-3.307-4.353-8.53-7.166-14.403-7.166z" fill="#FBBC05" />
      </g>
      <text x="50" y="34" fontSize="26" fontWeight="600" fill="white" fontFamily="'Geist Sans', system-ui, sans-serif">
        Google Cloud
      </text>
    </svg>
  );
}
